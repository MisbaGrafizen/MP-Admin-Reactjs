...
import {
  ...
  addPrePackageFoodItemAction,
  DeletePrePackageCategoryAction,
  deletePrePackageMethodByIdAction,
  EditPrePackageFoodItemAction,
...
export default function PrePackged({ methodType }) {
  ...
  const handleAddFoodItemSubmit = () => {
    ...
    if (isUpdateData) {
      ...
      dispatch(
        EditPrePackageFoodItemAction(foodItemInput._id, updatedFoodItemInput)
      )
        .then((response) => {
          setFoodItems((prev) =>
            prev.map((item) =>
  ...
  const handleCategoryUpdate = () => {
    if (inputValue && editingCategory) {
      dispatch(
        UpdatePrePackageCategoryNameAction(editingCategory, {
          name: inputValue,
        })
      )
        .then((response) => {
  ...
  const handelConfirmDelete = () => {
    if (isCategoryDelete) {
      dispatch(DeletePrePackageCategoryAction(categoriesDeleteId)).then(
        (response) => {
    ...
    if (file) {
      ...
      try {
        const response = await axios.post(
          "https://server.grafizen.in/api/v2/mp/admin/upload/prePackageFood",
          formData,
          {
            headers: {
