...
import {
  ...
  addFoodItemAction,
  addPrePackageFoodCategoryAction,
  DeleteSelfServingCategoryAction,
  deleteServingMethodByIdAction,
  EditSelfFoodItemAction,
...
export default function SelfServingManage({ methodType }) {
  ...
  useEffect(() => {
    if (selectedFoodCategory && selectedFoodCategory?._id) {
      if (methodType === "PRE-PACKGED") {
        dispatch(
          getPrePackageFoodItemByCategoryIdAction(selectedFoodCategory?._id)
        ).then((response) => {
          setFoodItems(response);
        });
      } else {
        dispatch(getFoodItemByCategoryIdAction(selectedFoodCategory?._id)).then(
          (response) => {
            setFoodItems(response);
          }
        );
      ...
      if (categoriesSingle) {
        if (methodType === "PRE-PACKGED") {
          dispatch(
            getPrePackageFoodItemByCategoryIdAction(categoriesSingle?._id)
          ).then((response) => {
            console.log("ifpar", response);
            setFoodItems(response);
          });
        } else {
          dispatch(getFoodItemByCategoryIdAction(categoriesSingle?._id)).then(
            (response) => {
  ...
  const handleCategorySubmit = () => {
    if (inputValue) {
      if (methodType === "PRE-PACKGED") {
        dispatch(addPrePackageFoodCategoryAction({ name: inputValue })).then(
          (response) => {
        ...
        dispatch(addFoodCategoryAction({ name: inputValue })).then(
          (response) => {
            setFoodCategories((prevCategories) => [
              ...prevCategories,
              response,
  ...
  const handleCategoryUpdate = () => {
    if (inputValue && editingCategory) {
      dispatch(
        UpdateSelfServicesCategoryNameAction(editingCategory, {
          name: inputValue,
        })
      )
        .then((response) => {
  ...
  const handelConfirmDelete = () => {
    if (isCategoryDelete) {
      dispatch(DeleteSelfServingCategoryAction(categoriesDeleteId)).then(
        (response) => {
    ...
    if (file) {
      ...
      try {
        const response = await axios.post(
          "https://server.grafizen.in/api/v2/mp/admin/upload/food",
          formData,
          {
            headers: {
  ...
  const handleUploadItemExcel = async (event) => {
    ...
    if (file) {
      ...
      try {
        const response = await axios.post(
          "https://server.grafizen.in/api/v2/mp/admin/food-items/excel",
          formData,
          {
            headers: {
              "Content-Type": "multipart/form-data",
        ...
        <div className="w-[100%] flex flex-col md150:gap-[30px] md11:gap-[20px]">
          <div className=" flex justify-between w-[100%]">
                    ...
                    <>
                      <div className="flex flex-col justify-center ">
                        ...
                        <p
                          ...
                          onClick={() => handleCategoryDelete(editingCategory)}
                        >
                          Delete
                        </p>
                      </div>
            ...
            <div className=" flex flex-col gap-[20px] justify-end ">
              <div
                className="w-[130px] bg-[#F28C28] text-white gap-[6px] border-[0.5px] border-[#000] font-[600] md150:text-[18px] md11:text-[15px]  md150:h-[40px] md11:h-[35px] flex justify-center items-center rounded-[8px] cursor-pointer"
                onClick={() =>
                  ...
                  {popupVisible && (
                    ...
                    >
                      <p
                        ...
                        onClick={() => handleEditFoodItem(selectedFoodItem)}
                      >
                        Edit
                      </p>
                      <p
          ...
          {(onClose) => (
            <>
              ...
              <div
                ...
                onClick={handleAddFoodItemSubmit}
              >
