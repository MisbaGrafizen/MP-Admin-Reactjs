...
export default function Menu({ methodType }) {
  ...
  useEffect(() => {
    if (selectedFoodCategory && selectedFoodCategory?._id) {
      if (methodType === "PRE-PACKGED") {
        ...
        dispatch(getItemByCategoryIdAction(selectedFoodCategory?._id)).then(
          (response) => {
            setFoodItems(response);
          }
        );
      ...
      if (categoriesSingle) {
        if (methodType === "PRE-PACKGED") {
          ...
          dispatch(getItemByCategoryIdAction(categoriesSingle?._id)).then(
            (response) => {
    ...
    if (file) {
      ...
      try {
        const response = await axios.post(
          "https://server.grafizen.in/api/v2/mp/admin/category/excel",
          formData,
          {
            headers: {
  ...
  const handleUploadItemExcel = async (event) => {
    ...
    if (file) {
      ...
      try {
        const response = await axios.post(
          "https://server.grafizen.in/api/v2/mp/admin/item/excel",
          formData,
          {
            headers: {
              "Content-Type": "multipart/form-data",
            ...
            {foodItems && foodItems.length > 0 ? (
              foodItems.map((item, index) => (
                ...
                >
                  ...
                  <div className="border-dashed pl-[7px] flex gap-[20px] p-[10px] rounded-[8px] border-[#fff] bg-[#F28C28]  w-full">
                    <div className="font-[600] pl-[7px] text-[15px] text-[white]">
                      <p>Name:</p>
                      <p>Price:</p>
                    </div>
