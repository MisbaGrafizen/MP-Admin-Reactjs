...
import {
  addBulkOrderCategoryAction,
  addBulkOrderFoodItemAction,
  addPrePackageFoodCategoryAction,
  deleteBulkOrderMethodByIdAction,
  DeleteBulkOrderCategoryAction,
  EditBulkOrderItemAction,
  getAllFoodCategoryAction,
  getAllPrePackageFoodCategoryAction,
  getBulkOrderFoodCategoryAction,
  getBulkOrderItemByCategoryIdAction,
...
export default function PrePackged({ methodType }) {
  ...
  useEffect(() => {
    if (selectedFoodCategory && selectedFoodCategory?._id) {
      if (methodType === "PRE-PACKGED") {
        ...
        dispatch(
          getBulkOrderItemByCategoryIdAction(selectedFoodCategory?._id)
        ).then((response) => {
          setFoodItems(response);
        });
      }
      ...
      if (categoriesSingle) {
        if (methodType === "PRE-PACKGED") {
          ...
          dispatch(
            getBulkOrderItemByCategoryIdAction(categoriesSingle?._id)
          ).then((response) => {
  ...
  const handleCategorySubmit = () => {
    if (inputValue) {
      if (methodType === "PRE-PACKGED") {
        ...
        dispatch(addBulkOrderCategoryAction({ name: inputValue })).then(
          (response) => {
            setFoodCategories((prevCategories) => [
              ...prevCategories,
              response,
  ...
  const handleCategoryUpdate = () => {
    if (inputValue && editingCategory) {
      dispatch(
        UpdateBulkOrderCategoryNameAction(editingCategory, { name: inputValue })
      )
        .then((response) => {
  ...
  const handelConfirmDelete = () => {
    if (isCategoryDelete) {
      dispatch(DeleteBulkOrderCategoryAction(categoriesDeleteId)).then(
        (response) => {
